...
class DQN:
    ...
    def __init__(self,n_actions,n_features,action_size=2, learning_rate=0.01,reward_decay=0.9,\
        ...
        self.memory = np.zeros((self.memory_size,n_features*2+action_size+1)) #build the memory structure

        #DQN consist of target_net and eval_net
        self._build_net()
        t_params = tf.get_collection('target_net_params')
        e_params = tf.get_collection('eval_net_params')
        #change the target net params by eval net param
    ...
    def _build_net(self):
        ...
        self.q_target = tf.placeholder(tf.float32,[self.action_size,self.n_actions], name='q_target')

        with tf.variable_scope('eval_net'):
            #c_names are collections to store variables
    ...
    def store_transition(self, s, a, r, s_):
        if not hasattr(self, 'memory_counter'):
            self.memory_counter = 0 #the items of the memory
