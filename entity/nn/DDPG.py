...
class DDPG(object):
    def __init__(self, action_space, state_space, action_upper_bound, **options):
        ...
        try:
            self.learning_rate = options['learning_rate']#学习效率
        except KeyError:
    ...
    def __build_critic(state, action, scope, trainable=True):
        ...
        w_init, b_init = tf.random_normal_initializer(.0, .3), tf.constant_initializer(.1)#正态分布初始化：均值、方差、常量初始化
...
def main(_):
    ...
    for episode in range(200):
        ...
        state, reward_episode = env.reset(), 0 #随机生成状态
